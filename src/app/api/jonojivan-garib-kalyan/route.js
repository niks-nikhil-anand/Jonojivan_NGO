  ...
  try {
    ...
    if (!memberName || !contactNumber || !documentNumber || !accountNumber) {
      ...
      return NextResponse.json(
        { 
          message: "Please provide member name, contact number, document number, and account number.",
    ...
    if (!photoUpload || !aadhaarCard || !bankPassbook) {
      ...
      return NextResponse.json(
        { 
          message: "Please upload all required documents: photo, Aadhaar card, and bank passbook.",
    ...
    if (!photoResult.secure_url || !aadhaarResult.secure_url || !bankResult.secure_url) {
    ...
    console.log("Garib Kalyan Yojana application submitted successfully:", newApplication._id);

    return NextResponse.json(
      { 
        message: "Garib Kalyan Yojana application submitted successfully! We will review your application and contact you soon.",
...
export const GET = async (req) => {
  try {
    ...
    const [applications, totalCount] = await Promise.all([
      GaribKalyanYojana.find(query)
        .sort({ createdAt: -1 }) // Latest first
        .skip(skip)
        .limit(limit)
        .lean(),
      GaribKalyanYojana.countDocuments(query)
    ]);
