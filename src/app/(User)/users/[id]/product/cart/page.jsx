...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    const userId = urlPath.split('/')[2]; // Define userId here

    const fetchCart = async () => {
      if (userId) { // Ensure id is available
        try {
          const response = await axios.get(`/api/users/cart/${userId}`);
          const fetchedCart = response.data.items; // Access items directly
          setCart(fetchedCart);
          localStorage.setItem('cart', JSON.stringify(fetchedCart)); // Set fetched cart to localStorage
        } catch (error) {
    ...
    const fetchProductDetails = async () => {
      ...
      try {
        const productDetails = await Promise.all(
          cart.map(async (item) => {
            const response = await axios.get(`/api/admin/dashboard/product/${item.productId}`); // Change `items` to `item`
  ...
  const incrementQuantity = (productId) => {
    ...
    const updatedCart = cart.map((item) =>
      item.productId === productId ? { ...item, quantity: item.quantity + 1 } : item // Fix: item.id to item.productId
    );
    setCart(updatedCart);
  ...
  const removeItem = (productId) => {
    const updatedProducts = products.filter((product) => product._id !== productId); // Fix: product.id to product._id
    setProducts(updatedProducts);
    
    // Also update localStorage
    const updatedCart = cart.filter((item) => item.productId !== productId); // Fix: item.id to item.productId
    setCart(updatedCart);
    localStorage.setItem('cart', JSON.stringify(updatedCart));
  };
